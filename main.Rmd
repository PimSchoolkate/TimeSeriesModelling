...
```{r}
validation(ARIMA.best)
validation(ARIMA.012.011)
```
...
#Intervals
tl<-ts(exp(pr-1.96*se),start=start,freq=12)
tu<-ts(exp(pr+1.96*se),start=start,freq=12)
pr<-ts(exp(pr),start=start,freq=12)
